...
const Logo = () => (
  <div className="flex flex-col items-center py-2 md:py-3 select-none group">
    <svg width="60" height="42" viewBox="0 0 160 110" fill="none" xmlns="http://www.w3.org/2000/svg" className="transform group-hover:scale-105 transition-transform duration-300 hidden md:block">
    ...
    <div className="flex items-center mt-0.5">
      <span className="text-lg md:text-xl font-black text-[#ec2027] pr-0.5">U</span>
      <span className="text-base md:text-lg font-black text-[#292667] tracking-tight">Book Store</span>
    </div>
  </div>
);
...
export const Header: React.FC<HeaderProps> = ({ schoolName, teacherCode, onMenuToggle }) => {
  return (
    <header className="sticky top-0 z-50 w-full bg-white border-b-2 border-[#fbee21] flex items-center shadow-sm shrink-0">
      <div className="max-w-7xl mx-auto px-4 md:px-6 w-full flex justify-between items-center h-auto min-h-[3.5rem] md:min-h-[4.5rem]">
        
        <div className="flex items-center gap-3">
          <button 
            onClick={onMenuToggle}
            className="lg:hidden p-1.5 rounded-lg text-[#292667] bg-slate-50 border border-slate-200 active:scale-95 transition-all"
          >
        ...
        <div className="flex items-center space-x-2">
          <nav className="hidden md:flex items-center space-x-1 lg:space-x-2">
            {['Home', 'Resources', 'Support'].map((item) => (
              <a key={item} href="#" className="text-[#292667] font-black text-xs lg:text-sm hover:text-[#ec2027] px-2 lg:px-4 py-1.5 rounded-full transition-all uppercase tracking-tighter">
                {item}
              </a>
            ))}
          </nav>
          
          <div className="flex items-center gap-2 md:gap-3 md:ml-4 md:pl-4 md:border-l border-slate-100">
            <div className="hidden sm:block text-right">
              <p className="text-[8px] md:text-[10px] font-black text-[#ec2027] uppercase tracking-widest leading-none">{schoolName}</p>
              <div className="flex items-center gap-1 justify-end mt-0.5">
                 <Star size={10} className="fill-[#fbee21] text-[#fbee21]" />
                 <p className="text-xs md:text-sm font-black text-[#292667] leading-none">{teacherCode}</p>
              </div>
            </div>
            <button className="p-1.5 md:p-2.5 bg-slate-50 text-[#292667] hover:bg-[#ec2027] hover:text-white rounded-lg transition-all shadow-sm relative group active:scale-90">
              <Bell size={18} md:size={20} strokeWidth={3} />
              <span className="absolute top-1 right-1 w-2 h-2 bg-[#00a651] rounded-full border border-white"></span>
            </button>
          </div>
        </div>
      </div>
